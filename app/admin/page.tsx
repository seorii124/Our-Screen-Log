          ...
          return (
            <div key={n} className={`pt-6 border-t border-gray-50 ${isMeOrAdmin ? '' : 'opacity-40 pointer-events-none'}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <input name={`m${n}_date`} type="text" value={(formData as any)[`m${n}_date`]} onChange={handleChange} disabled={!isMeOrAdmin} className="bg-gray-50 p-4 rounded-2xl disabled:bg-gray-100" placeholder="날짜" />
              </div>
            </div>
          );
        })}
