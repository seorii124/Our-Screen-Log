...
export default function MyLogPage() {
  ...
  useEffect(() => {
    async function fetchMyLogs() {
      ...
      // 이메일 하드코딩 제거 → metadata 사용
      const num = Number(user.user_metadata?.member) || 0
      setUser(user)
      setMemberNum(num)
  ...
  if (loading) return <div className="p-20 text-center font-black text-neutral-400">데이터를 불러오는 중...</div>
    ...
    <div className="max-w-7xl mx-auto p-10 min-h-screen pb-32">
      ...
      {works.length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-neutral-200 rounded-[3rem] bg-white">
          ...
          <Link href="/" className="text-blue-600 font-black text-xs uppercase underline">전체 보러가기</Link>
        </div>
      ) : (
